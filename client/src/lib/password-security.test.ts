...
describe("secureRandomInt", () => {
  ...
  test.each([0, -1, 1.5, Number.NaN, 0x1_0000_0001])(
  ...
  test("returns exact requested length and every requested family", () => {
    ...
    expect(
      [...password].some(character =>
        characterSets.lowercase.includes(character)
      )
    ).toBe(true);
    expect(
      [...password].some(character =>
        characterSets.uppercase.includes(character)
      )
    ).toBe(true);
    expect(
      [...password].some(character => characterSets.numbers.includes(character))
    ).toBe(true);
    expect(
      [...password].some(character => characterSets.symbols.includes(character))
    ).toBe(true);
  });
