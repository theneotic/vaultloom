...
afterEach(() => {
  ...
  if (originalEnvironment.BLOB_PUBLIC_BASE_URL === undefined)
    delete process.env.BLOB_PUBLIC_BASE_URL;
  else
    process.env.BLOB_PUBLIC_BASE_URL = originalEnvironment.BLOB_PUBLIC_BASE_URL;
});
...
describe("Vercel brand asset delivery", () => {
  it("maps public Blob storage to stable version-independent object paths", () => {
    process.env.BLOB_PUBLIC_BASE_URL =
      "https://vaultloom.public.blob.vercel-storage.com/";
    expect(getBrandAssetUrl("mark")).toBe(
      "https://vaultloom.public.blob.vercel-storage.com/vaultloom/brand/mark.png"
    );
    expect(getBrandAssetUrl("texture")).toBe(
      "https://vaultloom.public.blob.vercel-storage.com/vaultloom/brand/texture.png"
    );
  });
  ...
  it("keeps a managed-storage fallback only outside production", () => {
    ...
    expect(getBrandAssetUrl("generatorArt")).toMatch(
      /^\/manus-storage\/.+\.png$/
    );
